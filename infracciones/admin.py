...
class MultaAdmin(admin.ModelAdmin):
    list_display = ['numero_multa', 'vehiculo', 'monto_total', 'estado', 'fecha_vencimiento']
    list_filter = ['estado', 'fecha_generacion']
    search_fields = ['numero_multa', 'vehiculo__placa']
    readonly_fields = ['numero_multa', 'fecha_generacion']
    
    fieldsets = (
        ('Información de Multa', {
            'fields': ('numero_multa', 'infraccion', 'vehiculo', 'fecha_generacion')
        }),
        ('Montos', {
            'fields': ('monto_base', 'monto_total', 'descuento_pronto_pago', 'aumento_reincidencia')
        }),
        ('Fechas y Estado', {
            'fields': ('fecha_vencimiento', 'fecha_pago', 'estado')
        }),
    )
